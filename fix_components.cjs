...
function replaceColors(filePath) {
  ...
  content = content.replace(/shadow-\[0_0_40px_rgba\(255,78,0,0\.4\)\]/g, 'shadow-[0_0_40px_var(--color-primary)]');
  ...
  // Custom ones from component search
  content = content.replace(/bg-\[#0e0e0e\]/g, 'bg-background');
  content = content.replace(/border-\[#353534\]/g, 'border-outline');
  content = content.replace(/text-\[#e0e0e0\]/g, 'text-on-surface');
  
  fs.writeFileSync(filePath, content);
  console.log(`Updated ${filePath}`);
}

replaceColors('src/components/store/CheckoutModal.tsx');
replaceColors('src/components/store/CartDrawer.tsx');
