...
const oldRenderFunctionRegex = /const renderProductCard = \(product: Product\) => \([\s\S]*?<\/div>\n    <\/div>\n  \);/m;

newContent = newContent.replace(oldRenderFunctionRegex, `const renderProductCard = (product: Product) => (
    <ProductCard 
      key={product.id} 
      product={product} 
