...
code = code.replace(/{ \/\* Product Discovery[\s\S]*?\{\/\* Best Sellers \*\/}\s*\{bestSellers\.length > 0 && \(/, productDiscovery);

const newArrivals = `{/* New Arrivals */}
        {activeCategory === 'All' && newArrivals.length > 0 && (`;

code = code.replace(/\{\/\* New Arrivals \*\/\}\s*\{newArrivals\.length > 0 && \(/, newArrivals);
