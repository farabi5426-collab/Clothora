...
const getInvoiceStyles = () => `
  <style>
    ...
    .invoice-container .header-left {
      display: flex;
      align-items: center;
    }
    .invoice-container .header-left img {
      max-height: 50px;
    }
    .invoice-container .header-right {
      text-align: left;
      padding-left: 40px;
    }
    .invoice-container .header-right h1 {
      font-size: 36px;
      font-weight: 900;
      margin: 0 0 15px 0;
      letter-spacing: 1px;
    }
    .invoice-container .header-right p {
      margin: 5px 0;
      font-size: 14px;
      color: #CCCCCC;
    }
    .invoice-container .status-text {
      color: #FF4500;
      font-weight: bold;
    }
    ...
    .invoice-container .bill-to h2 {
      margin: 0 0 10px 0;
      font-size: 28px;
      font-weight: 800;
      color: #000;
    }
    .invoice-container .bill-to p {
      margin: 5px 0;
      font-size: 16px;
    }

    .invoice-container .meta-to {
      display: flex;
      flex-direction: column;
      gap: 20px;
    }
    .invoice-container .meta-item {
      display: flex;
      align-items: center;
      gap: 15px;
    }
    .invoice-container .meta-icon {
      color: #333333;
      display: flex;
    }
    .invoice-container .meta-label {
      color: #FF4500;
      font-size: 12px;
      font-weight: 800;
      letter-spacing: 1px;
    ...
    .invoice-container .items-table td:first-child { border-left: 1px solid #E5E7EB; }
    .invoice-container .product-cell {
      display: flex;
      align-items: center;
      gap: 20px;
    }
    .invoice-container .product-image {
      width: 80px;
      height: 100px;
      object-fit: cover;
      background: #f0f0f0;
    ...
    .invoice-container .product-details h3 {
      margin: 0 0 5px 0;
      font-size: 16px;
      font-weight: 800;
      color: #000;
    }
    .invoice-container .product-meta {
      color: #666666;
      font-size: 14px;
    }
    .invoice-container .qty-cell, .invoice-container .price-cell, .invoice-container .total-cell {
      text-align: center;
      font-size: 18px;
      font-weight: 600;
      color: #000;
    ...
    .invoice-container .delivery-card {
      background: #F9FAFB;
      padding: 20px 30px;
      border-radius: 8px;
      display: flex;
    ...
    .invoice-container .delivery-card svg {
      stroke: #FF4500;
    }
    .invoice-container .delivery-label {
      color: #FF4500;
      font-size: 12px;
      font-weight: 800;
      letter-spacing: 1px;
    ...
    .invoice-container .delivery-value {
      font-size: 16px;
      font-weight: 600;
      color: #000;
    }

    .invoice-container .totals-table {
      width: 350px;
      border-collapse: collapse;
    }
    ...
    .invoice-container .totals-table td.amount {
      text-align: right;
      font-weight: 600;
    }
    .invoice-container .totals-table .total-row td {
      border-top: 1px solid #E5E7EB;
      font-size: 24px;
      font-weight: 900;
      padding-top: 25px;
    }
    .invoice-container .totals-table .total-row td.amount {
      color: #FF4500;
    }

    /* THANK YOU BOX */
    .invoice-container .thank-you-box {
      background: #F9FAFB;
      padding: 30px 40px;
      border-radius: 8px;
      display: flex;
    ...
    .invoice-container .thank-you-content {
      display: flex;
      align-items: center;
      gap: 20px;
    }
    .invoice-container .shield-icon {
      background: #FFFFFF;
      padding: 10px;
      border-radius: 50%;
      display: flex;
    ...
    .invoice-container .thank-you-label {
      color: #FF4500;
      font-size: 12px;
      font-weight: 800;
      letter-spacing: 1px;
    ...
    .invoice-container .thank-you-text {
      font-size: 14px;
      line-height: 1.5;
      color: #333333;
    }
    .invoice-container .signature-box {
      text-align: right;
    }
    .invoice-container .signature-font {
      font-family: 'Dancing Script', cursive;
      font-size: 36px;
      color: #000;
      margin-bottom: 5px;
    }
    .invoice-container .signature-sub {
      font-size: 12px;
      color: #666666;
    }
    ...
    .invoice-container .footer-right svg {
      stroke: #FFFFFF;
    }
  </style>
`;
  ...
  <div class="invoice-container">
    <div class="header">
      <div class="header-left">
        <img src="${window.location.origin}/homepagelogo-removebg-preview.png" alt="CLOTHORA" class="logo" crossorigin="anonymous" />
      </div>
            ...
            <tr>
              <td>
                <div class="product-cell">
                  ${item.imageUrl ? `<img src="${item.imageUrl}" class="product-image" crossorigin="anonymous" />` : ''}
                  <div class="product-details">
                    <h3>${item.title}</h3>
                    <div class="product-meta">
                      ${item.selectedSize ? `Size: ${item.selectedSize}` : ''} ${(item.selectedSize && item.selectedColor && !item.selectedColor.startsWith('http')) ? ' | ' : ''} ${(item.selectedColor && !item.selectedColor.startsWith('http')) ? `Color: ${item.selectedColor}` : ''}
                    </div>
                  </div>
                </div>
              </td>
...
export const downloadInvoiceImage = async (order: any) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!invoiceElement) throw new Error("Invoice container not found");
