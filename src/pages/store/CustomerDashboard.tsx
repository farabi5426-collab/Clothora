...
export default function CustomerDashboard() {
  ...
  const [profileData, setProfileData] = useState({ name: '', phone: '', districtId: '', upazilaId: '', address: '' });
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    try {
      const districtName = districts.find(d => d.id === profileData.districtId)?.name || '';
      const upazilaName = upazilas.find(u => u.id === profileData.upazilaId)?.name || '';
    ...
    const fetchProfile = async () => {
      try {
        ...
        if (userDoc.exists()) {
          ...
          const dId = districts.find(d => d.name === data.district)?.id || '';
          const uId = upazilas.find(u => u.name === data.upazila)?.id || '';
          setProfileData({
          ...
          setProfileData({ name: user!.displayName || '', phone: '', districtId: '', upazilaId: '', address: '' });
        }
      } catch (e) {
          ...
          {activeTab === 'profile' && (
            <div className="bg-surface-container-low border-2 border-surface-bright p-[32px]">
              <form onSubmit={handleSaveProfile} className="space-y-[24px] max-w-lg">
                ...
                <div>
                  <label className="block text-[12px] uppercase tracking-[0.1em] font-bold text-on-surface-variant mb-[8px]">District</label>
                  <div className="relative">
                    <select value={profileData.districtId} onChange={e => setProfileData({...profileData, districtId: e.target.value, upazilaId: ''})} className="w-full bg-surface border-2 border-surface-bright p-[12px] text-[14px] text-on-background outline-none focus:border-primary font-bold appearance-none cursor-pointer">
                ...
                <div>
                  <label className="block text-[12px] uppercase tracking-[0.1em] font-bold text-on-surface-variant mb-[8px]">Upazila / Thana</label>
                  <div className="relative">
                    <select value={profileData.upazilaId} onChange={e => setProfileData({...profileData, upazilaId: e.target.value})} disabled={!profileData.districtId} className="w-full bg-surface border-2 border-surface-bright p-[12px] text-[14px] text-on-background outline-none focus:border-primary font-bold appearance-none cursor-pointer disabled:opacity-50">
                      <option value="" disabled>Select Upazila / Thana</option>
                      {upazilas.filter(u => u.district_id === profileData.districtId).map(u => (
